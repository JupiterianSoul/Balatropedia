...
import {
  ...
  CircleDollarSign, Spade, Skull, Ticket, Wand2, Settings as SettingsIcon, BookMarked, Star, FlaskConical,
  Swords, Hammer, Calculator, Hash, Trophy, Bell, Info,
} from "lucide-react";
...
const SECTIONS: HelpSection[] = [
  ...
  { key: "runchallenge", icon: <Swords className="h-3.5 w-3.5" /> },
  { key: "buildlab",     icon: <Hammer className="h-3.5 w-3.5" /> },
  { key: "calculator",   icon: <Calculator className="h-3.5 w-3.5" /> },
  { key: "seeds",        icon: <Hash className="h-3.5 w-3.5" /> },
  ...
  { key: "tierlist",     icon: <Trophy className="h-3.5 w-3.5" /> },
  ...
  { key: "whatsnew",     icon: <Bell className="h-3.5 w-3.5" /> },
  { key: "about",        icon: <Info className="h-3.5 w-3.5" /> },
  { key: "settings",     icon: <SettingsIcon className="h-3.5 w-3.5" /> },
];
...
export function HelpTab() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5" data-testid="tab-help">
      ...
      <Tabs value={active} onValueChange={setActive} orientation="vertical" className="grid gap-4 md:grid-cols-[220px_1fr]">
        <TabsList
